...
/*
 * Free dynamically allocated memory in the server struct.
 */
...
/*
 * Set up the address struct for the server socket. Similar to the example in assignment 5.
 */
 ...
 * the data length, reads the data from the client, decrypts the data, and sends
 * the decrypted data back to the client.
 */
 ...
 * decryption by subtracting the corresponding characters, and stores the
 * decrypted characters back in the buffer.
 */
void decryptData(struct Client *client) {
    // Calculate the length of the data (half of the total data length)
    int length = client->data_length / 2;
    // Variables to store the current data character, key character, and decrypted character
    int data_ch = 0;
    int key_ch = 0;
    int enc_ch = 0;
    ...
    for (int i = 0; i < length; i++) {
        ...
        data_ch -= 65;
        key_ch -= 65;
        // Perform the decryption by subtracting the key character from the data character
        enc_ch = data_ch - key_ch;
        // If the result is negative, adjust it to wrap around the alphabet
        if (enc_ch < 0) {
            enc_ch += 27;
        }
        // Convert the decrypted character back to its ASCII value
        enc_ch = (enc_ch % 27) + 65;
        // If the decrypted character is 91, set it to a space
        if (enc_ch == 91) {
            enc_ch = 32;
        }
        // Store the decrypted character back in the buffer
    ...
    if (argc < 2) {
        ...
        while (server->is_running) {
            ...
            if (client->socket < 0) {
                ...
                // Check if the token indicates a decryption request
                if (*(client->token) == 'd') {
